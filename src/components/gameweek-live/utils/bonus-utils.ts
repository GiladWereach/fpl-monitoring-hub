...
export const calculateBonusPoints = (playerBPSData: BPSData[], fixtureBPSData: BPSData[]): number => {
  ...
  const playersWithSameBps = sortedPlayers.filter(p => p.bps === playerBps).length;
  ...
  // Handle case where multiple players are tied for second
  if (uniqueBpsValues.length >= 2 && playerBps === sortedPlayers[1].bps) {
    return 2;
  }
  
  // Handle case where multiple players are tied for third
  if (uniqueBpsValues.length >= 3 && playerBps === sortedPlayers[2].bps) {
    return 1;
  }
