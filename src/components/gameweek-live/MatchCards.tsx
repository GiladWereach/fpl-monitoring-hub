...
const MatchCards = ({ gameweek }: MatchCardsProps) => {
  ...
  const getStatusColor = (match: any) => {
    if (match.started && !match.finished) return 'bg-green-500';
    if (!match.started) return 'bg-gray-500';
    if (match.finished) return 'bg-blue-500';
    return 'bg-red-500';
  };

  const getMatchStatus = (match: any) => {
    if (match.started && !match.finished) return 'LIVE';
    if (!match.started) return 'UPCOMING';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {matches?.map((match) => (
        <Card key={match.id} className="p-4 relative">
          <div className={`absolute top-2 right-2 w-3 h-3 rounded-full ${getStatusColor(match)}`} />
          ...
          <div className="flex flex-col space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Badge variant="outline">{getMatchStatus(match)}</Badge>
            </div>
